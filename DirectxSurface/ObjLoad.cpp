...
VECTOR4D			m_vscale = { 1.0,1.0,1.0,1 },
					m_vpos = { 0,0,0,1 },
					m_rot = { 0,0,0,1 };
...
//	Transform_RENDERLIST4DV1(&m_rend_list, &mrot, TRANSFORM_LOCAL_ONLY);
	Transform_OBJECT4DV1(&m_obj, &mrot, TRANSFORM_LOCAL_ONLY,1);


	////执行本地/模型到世界转换
	//Model_To_World_RENDERLIST4DV1(&m_rend_list, &m_poly1_pos);
	Model_To_World_OBJECT4DV1(&m_obj);


	////生成相机矩阵
	Build_CAM4DV1_Matrix_Euler(&m_cam, CAM_ROT_SEQ_ZYX);

	World_To_Camera_OBJECT4DV1(&m_obj, &m_cam);

	Camera_To_Perspective_OBJECT4DV1(&m_obj, &m_cam);

	Perspective_To_Screen_OBJECT4DV1(&m_obj, &m_cam);

	//Draw_RENDERLIST4DV1(&m_rend_list, m_imageData, WINDOW_WIDTH);

	Draw_OBJECT4DV1_Wire16(&m_obj, m_imageData, WINDOW_WIDTH);
}
...
void Game_Init()
{
	...
	Load_OBJECT4DV1_PLG(&m_obj, "tank1.plg", &m_vscale, &m_vpos, &m_rot);

	m_obj.world_pos.x = 0;
	m_obj.world_pos.y = 0;
	m_obj.world_pos.z = 100;
}
